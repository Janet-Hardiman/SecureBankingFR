...
public class OpenCVFaceRecognizer {
    public static void main(String[] args) {
        String trainingDir = "imageDir";//args[0];                               //should this just be name of directory????
        Mat testImage = imread("Janet001.png", CV_LOAD_IMAGE_GRAYSCALE);   //should this be list of files???

        File root = new File(trainingDir);

        FilenameFilter imgFilter = new FilenameFilter() {
            public boolean accept(File dir, String name) {          //taking directory and file name???
        ...
        for (File image : imageFiles) {
            int label;
            try (Mat img = imread(image.getAbsolutePath(), CV_LOAD_IMAGE_GRAYSCALE)) {
